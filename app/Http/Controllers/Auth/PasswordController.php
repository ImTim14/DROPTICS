...
class PasswordController extends Controller
{
    ...
    */
    ...
    public function getActivate(Request $request, $token = null)
    {
        ...
        $email = DB::table(config('auth.passwords.users.table'))->where('token', $token)->first()->email;
    ...
    public function getReset(Request $request, $token = null)
    {
        ...
        return view('global.auth', ['email' => $request->email, 'token' => $token]);
    }
    ...
    public function postActivate(Request $request)
    {
        ...
        DB::table(config('auth.passwords.users.table'))->where('token', $credentials['token'])->update(array(
        ...
        switch ($response) {
            case Password::PASSWORD_RESET:
                DB::table(config('auth.passwords.users.table'))->where('token', $credentials['token'])->delete();
    ...
    public function postEmail(Request $request)
    {
        ...
        $response = Password::sendResetLink($request->only('email'), function (CanResetPassword $user, $token) {
            $data = [
                'user' => $user,
                'token' => $token
            ];
            \Mail::send('emails.password', $data, function ($email) use ($user) {
                $email->to($user->email);
                $email->subject("Resetarea parolei");
            });
