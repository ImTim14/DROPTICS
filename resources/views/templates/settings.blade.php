...
@section('content')
    ...
    <div class="ui top attached tabular menu insura-tabs">
        <a class="item{{ session()->get('tab', 'profile') === 'profile' ? ' active' : null }}"
           data-tab="profile">{{ trans('settings.tab.menu.profile') }}</a>
        ...
        <a class="hide item{{ session()->get('tab', null) === 'security' ? ' active' : null }}"
           data-tab="security">{{ trans('settings.tab.menu.security') }}</a>
    </div>
    <div class="ui bottom attached tab segment{{ session()->has('tab') ? null : ' active' }}" data-tab="profile">
        <div class="row">
            <div class="col-md-6">
                ...
                <form action="{{ action('UserController@edit', array($user->id)) }}" enctype="multipart/form-data"
                      method="POST">
                    {{ csrf_field() }}
                    <div class="ui form">
                        <div class="field required">
                            ...
                            <input type="text" name="first_name"
                                   placeholder="{{ trans('settings.input.placeholder.first_name') }}" required
                                   value="{{ $user->first_name }}"/>
                        </div>
                        <div class="field">
                            <label>{{ trans('settings.input.label.last_name') }}</label>
                            <input type="text" name="last_name"
                                   placeholder="{{ trans('settings.input.placeholder.last_name') }}"
                                   value="{{ $user->last_name }}"/>
                        </div>
                        <div class="field required">
                            <label>{{ trans('settings.input.label.email') }}</label>
                            <input type="email" name="email"
                                   placeholder="{{ trans('settings.input.placeholder.email') }}" required
                                   value="{{ $user->email }}"/>
                        </div>
                        <div class="field">
                            <label>{{ trans('settings.input.label.phone') }}</label>
                            <input type="tel" placeholder="{{ trans('settings.input.placeholder.phone') }}"
                                   value="{{ $user->phone }}"/>
                        </div>
                        <div class="field">
                            <label>{{ trans('settings.input.label.birthday') }}</label>
                            <input type="date" class="datepicker" name="birthday"
                                   placeholder="{{ trans('settings.input.placeholder.birthday') }}"
                                   value="{{ $user->birthday }}"/>
                        </div>
                        <div class="field">
                            <label>{{ trans('settings.input.label.address') }}</label>
                            <input type="text" name="address"
                                   placeholder="{{ trans('settings.input.placeholder.address') }}"
                                   value="{{ $user->address }}"/>
                        </div>
{{--                        <div class="field required">--}}
...
{{--                                    <option{{ $user->locale === $language['locale'] ? ' selected' : '' }} value--}}
{{--                                    ="{{ $language['locale'] }}">{{ $language['name'] }}</option>--}}
{{--                                @endforeach--}}
{{--                            </select>--}}
{{--                        </div>--}}
                        <div class="field">
                            ...
                            <input type="file" accept="image/*" class="file-upload"
                                   data-allowed-file-extensions="bmp gif jpeg jpg png svg"
                                   data-default-file="{{ asset('storage/images/users/' . $user->profile_image_filename)
                                    }}"
                                   name="profile_image">
                        </div>
                        <div class="field">
                            <button class="ui right floated button primary m-w-140"
                                    type="submit">{{ trans('settings.button.save') }}</button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    <div class="ui bottom attached tab segment{{ session()->get('tab', null) === 'security' ? ' active' : '' }}"
         data-tab="security">
        <div class="row">
            <div class="col-md-6">
                    ...
                    <div class="ui form">
                        <div class="field required">
                            ...
                            <input type="password" name="current_password"
                                   placeholder="{{ trans('settings.input.placeholder.current_password') }}" required/>
                        </div>
                        <div class="field required">
                            <label>{{ trans('settings.input.label.new_password') }}</label>
                            <input type="password" name="new_password"
                                   placeholder="{{ trans('settings.input.placeholder.new_password') }}" required/>
                        </div>
                        <div class="field required">
                            <label>{{ trans('settings.input.label.confirm_password') }}</label>
                            <input type="password" name="new_password_confirmation"
                                   placeholder="{{ trans('settings.input.placeholder.confirm_password') }}" required/>
                        </div>
                        <div class="field">
                            <button class="ui right floated button primary m-w-140"
                                    type="submit">{{ trans('settings.button.save') }}</button>
