...
@section('profile_bar')
    ...
    <div class="ui segment white right-bar-profile">
        <div class="user-profile m-b-15">
            <div class="circular ui icon button top right pointing dropdown edit-profile-btn" data-inverted=""
                 data-tooltip="{{ trans('clients.menu.header.tooltip') }}" data-position="left center">
                <i class="icon ion-more"></i>
                <div class="menu">
                    <div class="header">{{ trans('clients.menu.header.text') }}</div>
                    <div class="divider"></div>
                    <div class="item">
                        <a href="#" data-target="#editClientModal" data-toggle="modal"><i
                                    class="write icon"></i> {{ trans('clients.menu.item.edit_profile') }}</a>
                    </div>
{{--                    <div class="item">--}}
...
{{--                    </div>--}}
                    <form action="{{ action('ClientController@delete', array($client->id)) }}" class="item negative"
                          method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
            ...
            @if ($client->profile_image_filename === 'default-profile.jpg')
                <div class="text-avatar"
                     style="background-color:{{ collect(config('insura.colors'))->random() }};">{{ strtoupper($client->first_name[0] . (isset($client->last_name) ? $client->last_name[0] : '')) }}</div>
            @else
                <img src="{{ asset('storage/images/users/' . $client->profile_image_filename) }}"
            ...
            <div class="m-t-25">
{{--                <button class="ui button primarish" data-target="#newTextModal"--}}
{{--                        data-toggle="modal"{{ is_null($user->company->text_provider) || is_null($client->phone) ? ' disabled' : '' }}>--}}
{{--                    <i class="comment icon"></i> {{ trans('clients.button.text') }} </button>--}}
                <button class="ui button primarish" data-target="#newEmailModal" data-toggle="modal"><i
                            class="mail icon"></i> {{ trans('clients.button.email') }} </button>
            </div>
        </div>
        <div class="scrollbar">
    ...
    <div class="half-page-content">
        ...
        <div class="ui segment white fs-16 client-details">
            <div class="segment-header">
                ...
                <div class="ui right floated successish button top m-w-140 right pointing dropdown" data-inverted=""
                     data-tooltip="{{ trans('clients.menu.header.tooltip') }}" data-position="left center">
...
{{--                        </div>--}}
        ...
        <div class="ui segment white fs-16">
            <div class="segment-header">
                ...
                <button class="ui right floated button successish m-w-140" data-target="#newNoteModal"
                        data-toggle="modal">{{ trans('clients.button.notes') }}</button>
            </div>
                ...
                @forelse ($client->notes as $note)
                    <tr>
                        ...
                        <td class="center aligned">
                            <a href="#" class="ui tiny grey label" data-target="#readNote{{ $note->id }}Modal"
                               data-toggle="modal"> {{ trans('clients.table.data.action.read') }} </a>
                            <form action="{{ action('NoteController@delete', array($note->id)) }}" class="delete-note"
                                  method="POST" style="display:inline;">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                <button href="#" class="ui red tiny label delete" style="cursor:pointer;"
                                        type="submit"> {{ trans('clients.table.data.action.delete') }} </button>
                            </form>
                        </td>
                    </tr>
                @empty
        ...
        <div class="ui segment white fs-16">
            <div class="segment-header">
                ...
                <button class="ui right floated button successish m-w-140" data-target="#newAttachmentModal"
                        data-toggle="modal">{{ trans('attachments.button.new') }}</button>
            </div>
            <table class="ui celled striped table">
                <thead>
                <tr>
                ...
                @forelse($client->attachments as $attachment)
                    <tr>
                        ...
                        <td class="center aligned">
                            <a href="{{ url('uploads/attachments/' . $attachment->filename) }}"
                               class="ui tiny grey label"
                               target="_blank"> {{ trans('clients.table.data.action.view') }} </a>
                            <form action="{{ action('AttachmentController@delete', array($attachment->id)) }}"
                                  class="delete-attachment" method="POST" style="display:inline;">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                <button class="delete label tiny red ui" style="cursor:pointer;"
                                        type="submit">{{ trans('clients.table.data.action.delete') }}</button>
                            </form>
                        </td>
                    </tr>
        ...
        <div class="ui segment white">
            <div class="segment-header">
                ...
                <button class="ui right floated button successish m-w-140" data-target="#newPolicyModal"
                        data-toggle="modal">{{ trans('clients.table.button.policies') }}</button>
            </div>
                    ...
                    <tr class="{{ $policy->statusClass }}">
                        ...
                        <td class="center aligned">
                            <a class="ui mini grey label"
                               href="{{ action('PolicyController@getOne', array($policy->id)) }}"> {{ trans('clients.table.data.action.view') }} </a>
                        </td>
                    </tr>
                @empty
                    <tr>
        ...
        <div class="ui segment white">
            <div class="segment-header">
                ...
                <button class="ui right floated button successish m-w-140" data-target="#newPaymentModal"
                        data-toggle="modal"{{ $policies->count() > 0 ? '' : ' disabled' }}>{{ trans('payments.button.new') }}</button>
            </div>
                ...
                <div class="ui form">
                    <div class="field required">
                        ...
                        <input type="text" name="subject"
                               placeholder="{{ trans('communication.input.placeholder.subject') }}" required
                               value="{{ old('subject') }}"/>
                    </div>
                    <div class="field required">
                        <label>{{ trans('communication.input.label.message') }}</label>
                        <textarea rows="5" name="message"
                                  placeholder="{{ trans('communication.input.placeholder.message') }}"
                                  required>{{ session('text') ? '' : old('message') }}</textarea>
                    </div>
                </div>
            </form>
        </div>
            ...
            <form action="{{ action('AttachmentController@add') }}" enctype="multipart/form-data" method="POST">
                ...
                <div class="ui form">
                    <div class="field required">
                        ...
                        <input type="text" name="name" placeholder="{{ trans('attachments.input.placeholder.name') }}"
                               required value="{{ old('name') }}"/>
                    </div>
                    <div class="field required">
                        <label>{{ trans('attachments.input.label.attachment') }}</label>
                        <input type="file" accept="image/*, application/pdf, .doc, .docx, .ppt, .pptx, .xls, .xlsx"
                               data-allowed-file-extensions="bmp doc docx gif jpeg jpg pdf png ppt pptx svg xls xlsx"
                               name="attachment" class="file-upload" required/>
                    </div>
                </div>
            </form>
        </div>
            ...
            <form action="{{ action('NoteController@add') }}" method="POST">
                ...
                <div class="ui form">
                    <div class="field">
                        ...
                        <textarea name="message" placeholder="{{ trans('notes.input.placeholder.message') }}" rows="5"
                        ...
                        <div class="field required">
                            ...
                            <select name="policy" class="ui fluid search dropdown" required>
                                @forelse ($policies->filter(function($policy) {
                                    ...
                                    <option data-policy-due="{{ $policy->due }}"
                                            {{ old('policy') == $policy->id ? 'selected' : null }} value="{{ $policy->id }}">{{ $policy->ref_no }}
                                        - {{ $policy->product->name }}</option>
                                @empty
                        ...
                        <div class="field required">
                            ...
                            <div class="ui labeled input">
                                ...
                                <input type="number" id="amount" name="amount"
                                       placeholder="{{ trans('payments.input.placeholder.amount') }}" required
                                       value="{{ old('amount') }}"/>
                            </div>
                        </div>
                    </div>
                    <div class="two fields">
                        <div class="field required">
                            ...
                            <div class="ui labeled input">
                                ...
                                <input type="text" class="datepicker" id="paymentDate" name="date"
                                       placeholder="{{ trans('payments.input.placeholder.date') }}" required
                                       value="{{ old('date') }}"/>
                            </div>
                        </div>
                        <div class="field required">
                            ...
                            <div class="ui selection dropdown">
                                ...
                                <div class="menu">
                                    <div class="item{{ old('method') === 'card' ? ' selected' : null }}"
                                         data-value="card">
                                        <i class="credit card alternative icon"></i>
                                        {{ trans('payments.input.option.method.card') }}
                                    </div>
                                    <div class="item{{ old('method') === 'cash' ? ' selected' : null }}"
                                         data-value="cash">
                                        <i class="money icon"></i>
                                        {{ trans('payments.input.option.method.cash') }}
                                    </div>
                                    <div class="item{{ old('method') === 'paypal' ? ' selected' : null }}"
                                         data-value="paypal">
                ...
                <div class="ui form">
                    <div class="two fields">
                        <div class="field required">
                            ...
                            <select name="product" class="ui fluid search dropdown" required
                                    value="{{ old('product') }}">
                                <option value="">{{ trans('policies.input.placeholder.product') }}</option>
                                @forelse ($client->company->products as $product)
                                    <option{{ old('product') === $product->id ? ' selected' : '' }} value
                                    ="{{ $product->id }}">{{ $product->name }}</option>
                                @empty
                                    <option disabled
                                            value="">{{ trans('policies.input.option.empty.product') }}</option>
                                @endforelse
                            </select>
                        </div>
                        <div class="field required">
                            ...
                            <select name="type" class="ui fluid search dropdown" required value="{{ old('type') }}">
                                ...
                                <option{{ old('type') === 'annually' ? ' selected' : '' }} value
                                ="annually">{{ trans('policies.input.option.type.annually') }}</option>
                                <option{{ old('type') === 'monthly' ? ' selected' : '' }} value
                                ="monthly">{{ trans('policies.input.option.type.monthly') }}</option>
                            </select>
                        </div>
                    </div>
                    <div class="two fields">
                        <div class="field">
                            ...
                            <div class="ui labeled input">
                                ...
                                <input type="text" id="expiry" class="datepicker" name="expiry"
                                       placeholder="{{ trans('policies.input.placeholder.expiry') }}" required
                                       value="{{ old('expiry') }}"/>
                            </div>
                        </div>
                        <div class="field">
                            ...
                            <div class="ui labeled input">
                                ...
                                <input type="text" id="renewal" class="datepicker" name="renewal"
                                       placeholder="{{ trans('policies.input.placeholder.renewal') }}" required
                                       value="{{ old('renewal') }}"/>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="two fields">
                        <div class="field required">
                            ...
                            <div class="ui labeled input">
                                ...
                                <input type="number" id="premium" min="0" name="premium[{{ $client->id }}]"
                                       placeholder="{{ trans('policies.input.placeholder.premium') }}" required
                                       value="{{ null != old('premium') ? old('premium')[$client->id] : null }}"/>
                            </div>
                        </div>
                        <div class="field">
                            <label>{{ trans('policies.input.label.payer') }}</label>
                            <input type="text" maxlength="64" name="payer"
                                   placeholder="{{ trans('policies.input.placeholder.payer') }}" required
                                   value="{{ old('payer') ?: $client->first_name . ' ' . $client->last_name }}"/>
                        </div>
                    </div>
                    <div class="divider"></div>
                    <div class="field required">
                        ...
                        <input type="text" name="beneficiaries"
                               placeholder="{{ trans('policies.input.placeholder.beneficiaries') }}"
                               value="{{ old('beneficiaries') }}"/>
                    </div>
                    <div class="field">
                        <label>{{ trans('policies.input.label.special_remarks') }}</label>
                        <textarea name="special_remarks"
                                  placeholder="{{ trans('policies.input.placeholder.special_remarks') }}"
                                  rows="4">{{ old('special_remarks') }}</textarea>
                    </div>
                    ...
                    <div class="three fields">
                        <div class="field">
                            ...
                            <div class="ui labeled input">
                                <label for="amount" class="ui label">{{ $client->currency_symbol }}</label>
                                <input type="number" id="amount" min="0" name="amount[{{ $client->id }}]"
                                       placeholder="{{ trans('policies.input.placeholder.amount') }}"
                                       value="{{ null != old('amount') ? old('amount')[$client->id] : null }}"/>
                            </div>
                        </div>
                        <div class="field">
                            <label>{{ trans('policies.input.label.payment_date') }}</label>
                            <input type="text" class="datepicker" name="date[{{ $client->id }}]"
                                   placeholder="{{ trans('policies.input.placeholder.payment_date') }}"
                                   value="{{ null != old('date') ?  old('date')[$client->id] : null }}"/>
                        </div>
                        <div class="field">
                            <label>{{ trans('policies.input.label.payment_method') }}</label>
                            <div class="ui selection dropdown">
                                <input type="hidden" name="method[{{ $client->id }}]"
                                       value="{{ null != old('method') ? old('method')[$client->id] : null }}"/>
