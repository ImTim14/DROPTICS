        ...
        <div class="scrolling content">
            ...
            <form action="{{ action('PolicyController@add') }}" method="POST">
                ...
                <div class="ui form">
                    ...
                    <div class="field required">
                        <label>{{ trans('policies.input.label.nr_inmat') }}</label>
                        <select class="dropdown fluid ui" name="nr_inmat" required>
                            <option value="">{{ trans('policies.input.label.nr_inmat') }}</option>
                            @forelse($nr_inmat as $nr)
                                <option{{ collect(old('nr_inmat'))->contains($nr->id) ? ' selected' : '' }} value="{{ $nr->id }}">{{ $nr->nr_inmat }}</option>
                            @empty
                                <option disabled value="">{{ trans('policies.input.option.empty.nr_inmat') }}</option>
                            @endforelse
                        </select>
                    </div>
                    <div class="field required">
                        <label>{{ trans('policies.input.label.ss') }}</label>
                        <select class="dropdown fluid ui" name="ss" required>
                            <option value="">{{ trans('policies.input.label.ss') }}</option>
                            @forelse($ss as $s)
                                <option{{ collect(old('ss'))->contains($s->id) ? ' selected' : '' }} value="{{ $s->id }}">{{ $s->ss }}</option>
                            @empty
                                <option disabled value="">{{ trans('policies.input.option.empty.ss') }}</option>
                            @endforelse
                        </select>
                    </div>
                    <div class="field">
