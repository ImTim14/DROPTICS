...
<head>
    ...
    <meta content="Droptics este un sistem de gestiune online pentru asigurări" name="description">
    <meta content="Dobroiu Andrei" name="author">
    <title>{{ trans('auth.title.' . $state) }} - {{ config('insura.name') }} | Droptics</title>
    <base href="{{ env('BASE_HREF', '') }}"/>
    <!-- Favicon -->
    <link href="{{ asset('favicon.ico') }}" rel="icon" sizes="16x16" type="image/x-icon">
    <link href="{{ asset('storage/images/' . config('insura.favicon')) }}" rel="icon"
          type="{{ mime_content_type(storage_path() . '/app/images/' . config('insura.favicon')) }}">
    <!-- Font and Icon Fonts -->
...
<div class="auth-card">
    <div class="auth-branding">
        <img src="{{ asset('storage/images/' . config('insura.favicon')) }}">
    </div>
    <div class="sign-in" style="{{ $state === 'login' ? '' : 'display:none;' }}">
        ...
        <form action="{{ action('Auth\AuthController@postLogin') }}" method="POST">
            ...
            <div class="ui form">
                <div class="field required">
                    ...
                    <input type="text" name="email" placeholder="{{ trans('auth.input.placeholder.email') }}" required
                           value="{{ old('email') }}">
                </div>
                <div class="field required">
                    <label>{{ trans('auth.input.label.password') }}</label>
                    <input type="password" name="password" placeholder="{{ trans('auth.input.placeholder.password') }}"
                           required>
                </div>
        ...
        <p>{{ trans('auth.request.register') }} <a href="" class="text-primary auth-action"
                                                   action="sign-up">{{ trans('auth.link.register') }}</a></p>
    </div>
    ...
    <div class="sign-up" style="{{ $state === 'register' ? '' : 'display:none;' }}">
                ...
                <div class="two fields">
                    <div class="field required">
                        ...
                        <input type="text" name="first_name"
                               placeholder="{{ trans('auth.input.placeholder.first_name') }}" required
                               value="{{ old('first_name') }}">
                    </div>
                    <div class="field">
                        <label>{{ trans('auth.input.label.last_name') }}</label>
                        <input type="text" name="last_name"
                               placeholder="{{ trans('auth.input.placeholder.last_name') }}"
                               value="{{ old('last_name') }}">
                    </div>
                </div>
                <div class="field required">
                    <label>{{ trans('auth.input.label.email') }}</label>
                    <input type="email" name="email" placeholder="{{ trans('auth.input.placeholder.email') }}" required
                           value="{{ old('email') }}">
                </div>
                <div class="field required">
                    <label>{{ trans('auth.input.label.company_name') }}</label>
                    <input type="text" name="company_name"
                           placeholder="{{ trans('auth.input.placeholder.company_name') }}" required
                ...
                <div class="two fields">
                    <div class="field required">
                        ...
                        <input type="password" name="password"
                               placeholder="{{ trans('auth.input.placeholder.password') }}" required>
                    </div>
                    <div class="field required">
                        <label>{{ trans('auth.input.label.password_confirmation') }}</label>
                        <input type="password" name="password_confirmation"
                               placeholder="{{ trans('auth.input.placeholder.password_confirmation') }}" required>
                    </div>
                </div>
                <div class="field">
        ...
        <p>{{ trans('auth.request.login') }} <a href="" class="text-primary auth-action"
                                                action="sign-in">{{ trans('auth.link.login') }}</a></p>
    </div>
        ...
        <div class="reset" style="">
            ...
            <form action="{{ action('Auth\PasswordController@postReset') }}" method="POST">
                ...
                <div class="ui form">
                    <div class="field required">
                        <label>{{ trans('auth.input.label.email') }}</label>
                        <input type="email" name="email" placeholder="{{ trans('auth.input.placeholder.email') }}"
                               required value="{{ $email ?? old('email') }}"/>
                    </div>
                    <div class="field required">
                        <label>{{ trans('auth.input.label.password_reset') }}</label>
                        <input type="password" name="password"
                               placeholder="{{ trans('auth.input.placeholder.password_reset') }}" required/>
                    </div>
                    <div class="field required">
                        <label>{{ trans('auth.input.label.password_confirmation') }}</label>
                        <input type="password" name="password_confirmation"
        ...
        <div class="activate" style="">
            ...
            <form action="{{ action('Auth\PasswordController@postActivate') }}" method="POST">
                ...
                <div class="ui form">
                    ...
                    <div class="field">
                        <button class="ui button fluid positive"
                                type="submit">{{ trans('auth.button.activate') }}</button>
                    </div>
                </div>
            </form>
